...
- Данные подгружаются с сервера с **лимитом 20 записей** за одну итерацию (если это возможно, если нет, то меньше).
...
- Я попытался все разбить архитектурно, поэтому в целом `Context api` вполне адекватно тут смотрится. Архитектура делится на:
  - **Контейнеры** — получают данные
  - **UI-компоненты** — отвечают за отображение
- Рассматривал `react-query`, даже был создан рабочий прототип, но с `IntersectionObserver` код становился чуть сложным в понимании, хотя да , базово там очень большой функционал для работы под такую задачу, почти все доступно из коробки.
